...
class ProxyControlledEmbedding extends Embedding {
    ...
    detectDimension(testText?: string): Promise<number> {
        return withSystemProxyPolicy(this.useSystemProxy, () => this.delegate.detectDimension(testText));
    }

    embed(text: string): Promise<EmbeddingVector> {
        return withSystemProxyPolicy(this.useSystemProxy, () => this.delegate.embed(text));
    }

    embedBatch(texts: string[]): Promise<EmbeddingVector[]> {
        return withSystemProxyPolicy(this.useSystemProxy, () => this.delegate.embedBatch(texts));
    }
...
export function createEmbeddingInstance(config: ContextMcpConfig): Embedding  {
    ...
    const previousProxyEnv = applySystemProxyPolicy(config.embeddingUseSystemProxy);
    try {
    ...
    return new ProxyControlledEmbedding(embedding, config.embeddingUseSystemProxy);
}
