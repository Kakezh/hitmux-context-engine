...
| `hce search <query> [path] [--limit n] [--scope all\|docs\|code]` | Recherche dans le contexte indexé depuis le shell. `scope` vaut `all` par défaut; utilisez `docs` ou `code` pour limiter les résultats. |
